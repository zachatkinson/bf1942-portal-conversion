...
def _process_node_volume(node: dict[str, Any], scene: ttype.Scene) -> bool:
    ...
    if node["tscn_type"] == "CollisionShape3D":
        ...
        if node["type"] == "":
            _logging.log_error(
                "CollisionShape3D node does not inherit from Volume type. Please add the Volume script in Godot."
            )
            _logging.log_error(f"Node: {node['name']}")
            return False
        if "shape" not in node:
            _logging.log_error(
                "Found CollisionShape3D node but with no corresponding shape. Unable to convert to json."
            )
            _logging.log_error(f"Node: {node['name']}")
            return False
        shape_data = next(
            (data for data in scene.flattened_list if data["id"] == node["shape"]), None
        )
        if not shape_data:
            _logging.log_error(
                "Found CollisionShape3D node but with no corresponding shape. Unable to convert to json."
        ...
        for point in point_raw:
            fb_point = [
                ...
                translation[1] - Decimal(0.5) * height,
                point[1] + translation[2],
            ]
            fb_points.append(putils.vector3_to_json(fb_point))
...
def _process_waypointpath(node: dict[str, Any], scene: ttype.Scene) -> bool:
    ...
    if not parent_node:
        ...
        return False
    if (
        "position" not in parent_node
        or "up" not in parent_node
        or "right" not in parent_node
        or "front" not in parent_node
    ):
    ...
    if prop != const.PROP_ID_WAYPOINTS:
        _logging.log_warning(
            f"processCurve3D: Curve3D parent {parent} has unexpected property name {prop}\n{str(node)}"
        )
    parent_transform = {
        "right": parent_node["right"],
        "up": parent_node["up"],
        "front": parent_node["front"],
        "position": parent_node["position"],
    }
    node["points"] = [
        putils.pos_to_parent_space(point, parent_transform) for point in node["points"]
    ]
    return True
