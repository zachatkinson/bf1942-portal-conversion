...
def fix_spatial_terrain(spatial_path: Path) -> None:
    ...
    for obj in data.get("Static", []):
        if "Terrain" in obj.get("type", ""):
            ...
            print(
                f"   📐 Found terrain at: ({terrain_offset_x:.2f}, {terrain_offset_y:.2f}, {terrain_offset_z:.2f})"
            )

            # Reset terrain to (0, 0, 0)
            obj["position"] = {"x": 0.0, "y": 0.0, "z": 0.0}
            print("   ✅ Reset terrain to: (0, 0, 0)")
            break
    ...
    print("   Terrain: (0, 0, 0)")
    print(
        f"   Objects shifted by: ({-terrain_offset_x:.2f}, {-terrain_offset_y:.2f}, {-terrain_offset_z:.2f})"
    )
