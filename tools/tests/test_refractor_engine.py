...
class TestRefractorCoordinateSystem:
    ...
    def test_get_scale_factor_returns_one(self):
        ...
        assert coord_system.get_scale_factor() == 1.0
...
class TestRefractorEngineSpawnPointIdentification:
    ...
    def test_is_spawn_point_with_spawn_and_point_in_name(self):
        ...
        assert self.engine._is_spawn_point("spawnpoint_axis_1", "")
    ...
    def test_is_spawn_point_with_spawnpoint_type(self):
        ...
        assert self.engine._is_spawn_point("some_object", "spawnpoint")
        assert self.engine._is_spawn_point("some_object", "objectspawner_spawnpoint")
    ...
    def test_is_spawn_point_with_instance_pattern(self):
        ...
        assert self.engine._is_spawn_point("openbasecammo_4_13", "")
        assert self.engine._is_spawn_point("spawnpoint_axis_1_1", "")
        assert self.engine._is_spawn_point("basecamp_3_5", "")

    def test_is_spawn_point_excludes_control_points(self):
        """Test control points are not identified as spawn points."""
        assert not self.engine._is_spawn_point("cpoint_base_1", "")
        assert not self.engine._is_spawn_point("controlpoint_cpoint_1", "")

    def test_is_spawn_point_rejects_non_spawn_objects(self):
        """Test non-spawn objects are rejected."""
        assert not self.engine._is_spawn_point("building_barn_01", "")
        assert not self.engine._is_spawn_point("vehicle_tank", "")
        assert not self.engine._is_spawn_point("tree_pine_large", "")
...
class TestRefractorEngineOwnershipClassification:
    ...
    def test_classify_spawn_ownership_team1_axis(self):
        ...
        assert self.engine._classify_spawn_ownership("spawnpoint_axis_1") == "team1"

    def test_classify_spawn_ownership_team1_index(self):
        """Test classifying team1 spawns by _1_ index pattern."""
        assert self.engine._classify_spawn_ownership("spawn_1_1") == "team1"
        assert self.engine._classify_spawn_ownership("basecamp_1_5") == "team1"

    def test_classify_spawn_ownership_team2_allies(self):
        """Test classifying team2 spawns by 'allies' keyword."""
        assert self.engine._classify_spawn_ownership("spawnpoint_allies_1") == "team2"

    def test_classify_spawn_ownership_team2_index(self):
        """Test classifying team2 spawns by _2_ index pattern."""
        assert self.engine._classify_spawn_ownership("spawn_2_1") == "team2"
        assert self.engine._classify_spawn_ownership("basecamp_2_3") == "team2"

    def test_classify_spawn_ownership_neutral(self):
        """Test classifying neutral spawns."""
        assert self.engine._classify_spawn_ownership("spawn_3_1") == "neutral"
        assert self.engine._classify_spawn_ownership("spawn_4_2") == "neutral"
        assert self.engine._classify_spawn_ownership("openbasecamp_5_1") == "neutral"
...
class TestRefractorEngineTeamFiltering:
    ...
    def test_should_include_spawn_for_team1(self):
        ...
        assert self.engine._should_include_spawn_for_team("team1", Team.TEAM_1) is True
        assert self.engine._should_include_spawn_for_team("team2", Team.TEAM_1) is False
        assert self.engine._should_include_spawn_for_team("neutral", Team.TEAM_1) is False

    def test_should_include_spawn_for_team2(self):
        """Test including team2 spawns for team2 request."""
        assert self.engine._should_include_spawn_for_team("team2", Team.TEAM_2) is True
        assert self.engine._should_include_spawn_for_team("team1", Team.TEAM_2) is False
        assert self.engine._should_include_spawn_for_team("neutral", Team.TEAM_2) is False
...
class TestRefractorEngineCoordinateConversion:
    ...
    def test_get_coordinate_system_returns_refractor_system(self):
        ...
        assert isinstance(coord_system, RefractorCoordinateSystem)
