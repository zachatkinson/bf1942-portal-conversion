...
from bfportal.terrain.terrain_provider import MeshTerrainProvider, HeightAdjuster
...
class PortalConverter:
    ...
    def __init__(self, args):
        ...
        mappings_path = self.project_root / 'tools' / 'asset_audit' / 'bf1942_to_portal_mappings.json'

        self.asset_mapper = AssetMapper(portal_assets_path)
        self.asset_mapper.load_mappings(mappings_path)
        print(f"   ✅ Asset mapper loaded (733 mappings)")

        # Coordinate offset calculator
        self.coord_offset = CoordinateOffset()
        print(f"   ✅ Coordinate offset calculator")

        # Terrain provider: Load Portal base terrain mesh
        terrain_mesh_path = self.project_root / 'GodotProject' / 'raw' / 'models' / f'{args.base_terrain}_Terrain.glb'
        ...
        print(f"      Height range: {self.terrain.min_height:.1f}m - {self.terrain.max_height:.1f}m")
        print(f"      Mesh bounds: X[{self.terrain.mesh_min_x:.1f}, {self.terrain.mesh_max_x:.1f}] "
              f"Z[{self.terrain.mesh_min_z:.1f}, {self.terrain.mesh_max_z:.1f}]")
        print(f"      Mesh center: ({self.terrain.mesh_center_x:.1f}, {self.terrain.mesh_center_z:.1f})")
        ...
        try:
            ...
            if all_objects:
                ...
                for spawn in map_data.team1_spawns + map_data.team2_spawns:
                    objects_for_centroid.append(GameObject(
                        name=spawn.name,
                ...
                print(f"   Source center: ({source_center.x:.1f}, {source_center.y:.1f}, {source_center.z:.1f})")
                print(f"   Target center: ({target_center.x:.1f}, {target_center.y:.1f}, {target_center.z:.1f})")
            ...
            print(f"   Source: {source_analysis.orientation.value.upper()} "
                  f"({source_analysis.width_x:.0f}m x {source_analysis.depth_z:.0f}m, "
                  f"ratio: {source_analysis.ratio:.2f})")
            print(f"   Terrain: {dest_analysis.orientation.value.upper()} "
            ...
            if rotation_result.rotation_needed:
                print(f"   ⚠️  Will rotate terrain {rotation_result.rotation_degrees}° around Y axis")
            ...
            if skipped_terrain:
                ...
                print(f"   Portal SDK does not support as placeable objects. Water bodies are")
                print(f"   part of the base terrain system and cannot be added dynamically.")
                print(f"   Hopefully future Portal updates will add better water support.")

            print(f"\nNext steps:")
            print(f"  1. Open {output_path} in Godot")
            print(f"  2. Review object placements")
            print(f"  3. Export via BFPortal panel")
    ...
    def _resolve_map_path(self) -> Path:
        ...
        if self.args.bf1942_root:
            ...
            base_path = self.project_root / 'bf1942_source' / 'extracted' / 'Bf1942' / 'Archives' / 'bf1942' / 'Levels'
    ...
    def _generate_tscn(self, map_data, output_path: Path):
        ...
        try:
            ...
            print(f"   ✅ Production .tscn generated with full Portal structure")
        except Exception as e:
            print(f"   ⚠️  Error using production generator: {e}")
            print(f"   Falling back to basic stub...")
            import traceback
            traceback.print_exc()
            ...
            with open(output_path, 'w') as f:
                ...
                f.write(f"# Converted from BF1942\n")
...
Note:
  ...
  vertical positioning with proper collision detection against the terrain mesh.
    ...
    parser.add_argument('--map', required=True,
                       help='Map name (e.g., Kursk, Wake_Island)')
    parser.add_argument('--base-terrain', required=True,
                       help='Portal base terrain (e.g., MP_Tungsten, MP_Outskirts)')
    parser.add_argument('--bf1942-root', type=str,
                       help='Path to BF1942 extracted maps (default: bf1942_source/extracted/...)')
    parser.add_argument('--output', type=str,
                       help='Output .tscn path (default: GodotProject/levels/<map>.tscn)')
    parser.add_argument('--terrain-size', type=float, default=2048.0,
