...
from ..core.interfaces import (
    ISceneGenerator, MapData, GameObject, SpawnPoint, CapturePoint,
    Transform, Vector3, Rotation, Team
)
...
class TscnGenerator(ISceneGenerator):
    ...
    def generate(self, map_data: MapData, output_path: Path, base_terrain: str = "MP_Tungsten") -> None:
        ...
        for resource in self.ext_resources:
            lines.append(f'[ext_resource type="{resource["type"]}" '
        ...
        vehicle_spawners = [obj for obj in map_data.game_objects
                           if 'vehicle' in obj.asset_type.lower() or
                              'spawner' in obj.asset_type.lower()]
        if vehicle_spawners:
    ...
    def _validate_map_data(self, map_data: MapData) -> None:
        ...
        if len(map_data.team1_spawns) < 4:
            raise ValidationError(f"Team 1 needs at least 4 spawns, got {len(map_data.team1_spawns)}")
        if len(map_data.team2_spawns) < 4:
            raise ValidationError(f"Team 2 needs at least 4 spawns, got {len(map_data.team2_spawns)}")
    ...
    def _format_transform(self, transform: Transform) -> str:
        ...
        return (f"Transform3D({m00:.6g}, {m01:.6g}, {m02:.6g}, "
    ...
    def _generate_hqs(self, map_data: MapData) -> List[str]:
        ...
        lines.append(f'[node name="TEAM_1_HQ" parent="." node_paths=PackedStringArray("HQArea", "InfantrySpawns") instance=ExtResource("1")]')
        ...
        for i, spawn in enumerate(team1_spawns, 1):
            lines.append(f'[node name="SpawnPoint_1_{i}" parent="TEAM_1_HQ" instance=ExtResource("2")]')
        ...
        lines.append(f'[node name="TEAM_2_HQ" parent="." node_paths=PackedStringArray("HQArea", "InfantrySpawns") instance=ExtResource("1")]')
        ...
        for i, spawn in enumerate(team2_spawns, 1):
            lines.append(f'[node name="SpawnPoint_2_{i}" parent="TEAM_2_HQ" instance=ExtResource("2")]')
    ...
    def _generate_capture_points(self, capture_points: List[CapturePoint]) -> List[str]:
        ...
        for i, cp in enumerate(capture_points, 1):
            # Build spawn arrays
            team1_spawn_paths = [f'NodePath("CP{i}_Spawn_1_{j+1}")' for j in range(len(cp.team1_spawns))]
            team2_spawn_paths = [f'NodePath("CP{i}_Spawn_2_{j+1}")' for j in range(len(cp.team2_spawns))]
            ...
            lines.append(f'points = PackedVector2Array(-{radius}, -{radius}, {radius}, -{radius}, {radius}, {radius}, -{radius}, {radius})')
            lines.append('')

            # Generate spawn point nodes as children of the capture point
            for j, spawn in enumerate(cp.team1_spawns, 1):
                lines.append(f'[node name="CP{i}_Spawn_1_{j}" parent="CapturePoint_{i}" instance=ExtResource("2")]')
                rel_transform = self._make_relative_transform(spawn.transform, cp.transform)
                lines.append(f'transform = {self._format_transform(rel_transform)}')
                lines.append('')

            for j, spawn in enumerate(cp.team2_spawns, 1):
                lines.append(f'[node name="CP{i}_Spawn_2_{j}" parent="CapturePoint_{i}" instance=ExtResource("2")]')
                rel_transform = self._make_relative_transform(spawn.transform, cp.transform)
                lines.append(f'transform = {self._format_transform(rel_transform)}')
                lines.append('')
    ...
    def _generate_combat_area(self, map_data: MapData) -> List[str]:
        ...
        lines.append('[node name="CombatArea" parent="." node_paths=PackedStringArray("CombatVolume") instance=ExtResource("5")]')
        lines.append(f'transform = Transform3D(1, 0, 0, 0, 1, 0, 0, 0, 1, {center_x}, {center_y}, {center_z})')
        ...
        lines.append(f'points = PackedVector2Array({-half_width}, {-half_depth}, '
    ...
    def _generate_static_layer(self, map_data: MapData) -> List[str]:
        ...
        if terrain_rotation != 0:
            ...
            # [0,   1, 0  ]
            # [-sin, 0, cos]
            terrain_transform = (f"Transform3D({cos_r:.6g}, 0, {sin_r:.6g}, "
                               f"0, 1, 0, "
                               f"{-sin_r:.6g}, 0, {cos_r:.6g}, "
                               f"0, 0, 0)")

            lines.append(f'[node name="{self.base_terrain}_Terrain" parent="Static" instance=ExtResource("6")]')
        ...
        static_objects = [obj for obj in map_data.game_objects
                         if 'vehicle' not in obj.asset_type.lower() and
                            'spawner' not in obj.asset_type.lower() and
                            'spawnpoint' not in obj.asset_type.lower() and
                            'spawn' not in obj.asset_type.lower() and
                            'controlpoint' not in obj.asset_type.lower() and
                            'capturepoint' not in obj.asset_type.lower() and
                            'cpoint' not in obj.asset_type.lower()]
